...
def _get_dynamic_binary_image(filedir, img_name):
    ...
    filename = './out_img/' + img_name.split('.')[0] + '.jpg'
...
def main():
    ...
    for file in os.listdir(filedir):
        ...
        for i in range(cutting_img_num):
            try:
                file = './out_img/%s-cutting.jpg' % (img_name.split('.')[0], i)
                print('文件名'+file)
                tessdata_dir_config = r'--tessdata-dir "D:Tesseract-OCR\tessdata"'
                # 识别验证码
                str_img = pytesseract.image_to_string(Image.open(file), config=tessdata_dir_config)  # 单个字符是10，一行文本是7
                print('heiheihei'+str_img)
